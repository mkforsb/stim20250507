...
func (report PayoutReport) filterSum(fn func(entry PayoutEntry) bool) uint64 {
	var total uint64
...
type PayoutReportParser interface {
	TryParseUrl(url string) (PayoutReport, error)
	TryParseStream(stream io.Reader) (PayoutReport, error)
}
...
type AlternativePayoutReportParser struct{}

func (parser AlternativePayoutReportParser) TryParseUrl(url string) (PayoutReport, error) {
	return PayoutReport{}, errors.New("This parser is unimplemented.")
}

func (parser AlternativePayoutReportParser) TryParseStream(stream io.Reader) (PayoutReport, error) {
	return PayoutReport{}, errors.New("This parser is unimplemented.")
}

type FirstSuccessfulPayoutReportParser struct {
	parsers []PayoutReportParser
}

func (parser FirstSuccessfulPayoutReportParser) TryParseUrl(url string) (PayoutReport, error) {
	// Potential optimization: fetch full data from URL and call innerParser.TryParseStream on byte
	//  buffer. Could be separate type, e.g CachingFirstSuccessfulPayoutReportParser ..
	for _, innerParser := range parser.parsers {
		result, err := innerParser.TryParseUrl(url)
	...
	return PayoutReport{}, errors.New("No parser succeeded.")
}

func (parser FirstSuccessfulPayoutReportParser) TryParseStream(stream io.Reader) (PayoutReport, error) {
	for _, innerParser := range parser.parsers {
		result, err := innerParser.TryParseStream(stream)
...
func main() {
	...
	parser := FirstSuccessfulPayoutReportParser{[]PayoutReportParser{
		AlternativePayoutReportParser{},
		StrictCSVPayoutReportParser{},
	}}
	...
	for n := range target_range_end - target_range_start + 1 {
		...
		total += report.filterSum(func(entry PayoutEntry) bool {
